...
namespace DfE.EducationProviderRegistry.Web.SharedTests.Infrastructure.Antiforgery.Extensions;

public static class HttpClientExtensions
{
    public static async Task<AntiForgeryContext> GetAntiforgeryTokensAsync(
        this HttpClient client,
        CancellationToken cancellationToken = default)
    {
        using HttpResponseMessage response =
            await client.GetAsync("/_test/antiforgery", cancellationToken);

        response.EnsureSuccessStatusCode();

        AntiForgeryRequestToken dto =
            await response.Content.ReadFromJsonAsync<AntiForgeryRequestToken>(cancellationToken) ??
